...
void run_sturmian14(double Z, double k_exp, size_t n_alpha, size_t n_beta) {
    ...
    std::ofstream mathematicafile("/tmp/debug_gscf_sturm14.m");
    ...
    // Define SCF types:
    typedef PlainScfHartreeFock<fock_type> scfhf_type;
    typedef typename scfhf_type::scf_control_type scf_control_type;

    // Allocate SCF objects:
    scf_control_type scf_control;
    scfhf_type scfhf(scf_control, debugout);
    scfhf.solve_assert(fock, idata.s_bb());
}
}  // namespace scf_dummy
