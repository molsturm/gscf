...
public:
  ...
  virtual scf_state_type solve(probmat_type probmat_bb,
                               const matrix_type& metricmat_bb) const = 0;
...
ScfBase<ScfState>::solve_assert(probmat_type probmat_bb,
                                const matrix_type& metricmat_bb) const {
  scf_state_type state = solve(std::move(probmat_bb), metricmat_bb);
...
inline ScfBase<ScfState>::ScfBase(
      const ScfControlBase<scf_traits_type>& scf_control)
      : m_scf_control_ptr{
...
void ScfBase<ScfState>::solve_eigensystem(scf_state_type& s) const {
  ...
  // Do eigenproblem:
  bool res = detail::eig_sym_hack(*s.problem_matrix_ptr(), s.metric_matrix(),
