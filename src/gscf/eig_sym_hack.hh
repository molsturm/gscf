...
bool eig_sym_hack(
      ...
      const typename ProblemMatrix::stored_matrix_type& overlapmat_bb,
    ...
    assert_dbg(overlapmat_bb.is_symmetric(), ExcMatrixNotSymmetric());
    ...
    // Take inverse square root of overlap:
    ArmadilloMatrix<double> invsqrt_overlap = invsqrtmat_hack(overlapmat_bb);

    // Assert that this is symmetric:
    assert_dbg(invsqrt_overlap.is_symmetric(), ExcMatrixNotSymmetric());

    // Transform the fock matrix into the orthogonal basis:
    // Assume that invsqrt_overlap is hermetian (as it should)
    auto fock_transformed = invsqrt_overlap * (f * invsqrt_overlap);
    ...
    evec = invsqrt_overlap * ArmadilloMatrix<double>(evec_arma.t());
